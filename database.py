...
# Columnas: ID, Nombre, Lat, Lng, Distrito, Dirección, Tipo, Cara, Ancho, Alto, Tráfico, Precio, NSE, Demografía, Puntuación
SAMPLE_PANELS = [
...
def get_paneles_con_estado():
    ...
    conn = get_conn()
    query = """
        SELECT p.*, c.fecha_fin, cl.empresa AS cliente, c.nombre_campana 
        FROM paneles p
    ...
    today = date.today()
    def calc_status(row):
        if not row["fecha_fin"]: return "libre"
    ...
    df["dias_restantes"] = df["fecha_fin"].apply(lambda f: (date.fromisoformat(str(f)) - today).days if f else None)
    return df
